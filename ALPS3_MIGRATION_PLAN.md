...
| 2026-09-24 | 2.3A | Warning cleanup and no-output-change fixes complete: Eigen index types, initializer order, unused/deprecated code, inline HDF5/LAPACK helpers, debug size check, unreachable kernel branches, robust text-file extraction, and dead bindings/config code removed. The dev preset enables `MAXENT_WERROR`; 43/43 Release tests pass including full regression, and the LAPACK build is warning-free. |
